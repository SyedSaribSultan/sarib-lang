...
**Evidence:** `bench/scale-report.md` (same-machine worktree before/after — worst baseline slope **~2.0–2.3** vs worst current **~1.2**, across runs; G9 capacity 4,000 → 30,000 nodes per `bench/gate-report.md`); `impl/tests/run_golden.py` (155 query fingerprints, 11 op states, 9 rejection messages byte-identical); `impl/tests/test_index_fuzz.py` (**11,831** ops applied by default — the same command CI runs — zero divergence, with a committed negative control that disables `Doc.touch()` and requires the net to report incoherence, so a passing run cannot mean "cannot fail"); `plans/01-scale-remediation.md`; P17, D-029, D-044.

---

## D-064 · G4's budget measures the conformance surface, not the transports
**Date:** 2026-07-28 · **Stage:** post-15 execution (scale remediation) · **Status:** Ratified

**Context:** G4 exists to keep success test S6 honest — *"a conforming parser is a weekend's work"* — by capping the reference implementation at 1000 non-blank/non-comment lines. But it counted `cli.py` (247) and `mcp_server.py` (101) alongside the format itself. Nothing in the specification requires a CLI or an MCP server; they are transports we happen to ship. After the scale fix (+54 LOC) the total hit 987/1000, leaving 13 lines — so a mis-scoped gate was about to force real features out of the core for no principled reason. Filed as RM23.
**Options:** (a) leave the budget as-is and absorb the 13-line ceiling; (b) raise 1000 to a larger number; (c) narrow the budget to what S6 actually claims and report the rest.
**Choice:** (c). The **budgeted** figure is now the conformance surface only — `parser + canon + model + ops + query + render + __init__` = **658/1000**. Transports (348) and consumers (`importer`, `preview`, 458) are still measured and printed on every run, plus a whole-package total, so the narrower gate hides nothing. `bench/run_gates.py` additionally **fails if any module is unclassified**, so a new file cannot escape the budget by not being listed. Option (b) was rejected outright: raising the number to fit the code is how a gate becomes decoration.
**Reversal condition:** if a third-party implementer reports that reaching conformance required substantially more than the budgeted surface — i.e. the CLI/MCP split turned out to be load-bearing for interoperability after all — fold the transports back in and re-argue the ceiling.
**Evidence:** `bench/gate-report.md` (G4 section, three tiers); register RM23; `plans/01-scale-remediation.md` §5 D4; S6 in `stages/01-vision-and-philosophy.md` §5.

---

## D-065 · Op-time validation is scoped to the ids an op touched
**Date:** 2026-07-28 · **Stage:** post-15 execution (scale remediation) · **Status:** Ratified

**Context:** `ops.apply()` ran a full-document `check_invariants()` after every operation and rejected if it found *anything*. Consequence: a file that already carried an unrelated violation — two headings sharing one `{#slug}`, which is hand-authorable — rejected **every** operation, including the one that would repair it. An author could be locked out of their own file. A narrowed check had been prototyped earlier in the same session, measured for *speed*, found to give no speed win, and reverted; that judged it on the wrong axis, since its real value is correctness.
**Options:** (a) leave it, and accept that a damaged file is un-editable through the op vocabulary; (b) reject only violations the op newly introduced, by diffing before/after (correct, but doubles the per-op cost); (c) validate only the ids the op touched.
**Choice:** (c). `Doc.check_touched(ids)` checks parent-existence and ancestor-chain termination for the touched ids. That is the entire surface an op can break: no op accepts a slug, and each of `create-node` / `move` / `add-edge` / `merge` guards its endpoints **inline and raises before mutating** (a missing-`into` guard was added to `merge` for this). Full-document `check_invariants()` remains, and is what `sarib validate` and load-time run. Every rejection message a caller can observe originates from an inline guard, so the 9 pinned rejection messages in the golden net are unchanged.
**Reversal condition:** if any invariant violation is observed escaping op-time detection and reaching a persisted file, revert to full-document validation per op and pay the cost.
**Evidence:** `impl/tests/test_op_validation.py` (16 checks: a pre-existing duplicate slug no longer blocks edits, the repairing edit is accepted, and all 9 guards still reject with their messages); `impl/tests/run_golden.py` unchanged (124 query fingerprints, 11 op states, 9 rejection messages); RM14; `plans/01-scale-remediation.md` §10 F2.
