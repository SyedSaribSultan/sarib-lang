...
| RA5 | Derived-relation query cost too high → tempts materialization | M | M | B (Stage 6) | transitive-closure queries slow on big graphs | Mandatory traversal bounds (D-028) cap closure cost; provenance-marked caches (D-022). **Measured 2026-07-28** (`bench/scale-report.md`, gate G9): the bound capped *output* but not *work* — the graph walk re-sorted the entire edge table per visited node (O(V·E log E)). Now adjacency-indexed and linear (slope 1.14, baseline 2.02); 443ms at 124,841 nodes / 24,841 edges. Residual: `select:none` is still output-bounded only (follow-up F3) | 🟡 mitigated |
