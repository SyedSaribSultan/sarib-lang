...
| **RM8** | The plain-text-as-truth bet loses to a database | The only fully-working single-source-many-views system (Notion) is a DB ([tools-for-thought.md](../research/tools-for-thought.md)); if files can't scale, the core stance is wrong | Phase B/C perf modeling; reference-impl query/write latency at 100k nodes | Accept as a deliberate bet (ownership/longevity/carrier); op-log (P11) makes the fallback cheap — file demotes to append-only journal + live index without model change (R-new-1 → accepted). **Signal fired and was cleared, 2026-07-28:** the reference impl was quadratic and unusable by 10k nodes — *below* this row's 100k trigger — because it had no index at all. Remediated (`plans/01-scale-remediation.md`); ~125k nodes now parse in ~1s, canonicalize in ~0.8s, and answer a bounded query in ~40ms, in-process, no DB. The bet is **not** currently losing; the earlier reading was an implementation defect, not evidence about plain text. |
...
| RM8 | Plain-text-as-truth loses to a DB at scale | M | H | B/C | query/write latency unacceptable at 100k nodes | Op-log fallback to live index (R-new-1); see Watch list. **Measured 2026-07-28** (`bench/scale-report.md`, gate G9): at ~125k nodes parse ~1s · canon ~0.8s · bounded query ~40ms · full graph walk ~0.4s · ~92MB model + ~11MB derived index from a 7.2MB file (exact per-run figures in the report). Trigger not met | 🟠 accepted |
...
| RM19 | Canonical-form recomputation cost — re-canonicalizing a large file on every change | M | L | C (Stage 9) | serialization dominates edit latency at scale | Edits run against the append-only op-log (cheap); canonicalization off-hot-path; incremental canonicalization possible (D-042). **Measured 2026-07-28** (`bench/scale-report.md`, gate G9): canon was O(N²) via the walk (baseline slope ~1.9–2.0) and is now linear (~1.05) — ~0.8s at ~125k nodes. Still a whole-file recompute; incremental remains unbuilt | 🟡 mitigated |
...
| RM23 | **G4 LOC headroom exhausted** — the core is 987/1000 after the scale fix (+54), leaving 13 lines. The "conforming parser is a weekend's work" claim (S6) is load-bearing for the whole thesis | M | M | E (freeze) | the next core feature forces a budget argument, or the budget quietly rises | Do **not** raise the budget silently: either tighten, or re-argue G4's scope openly (CLI+MCP are transports, not conformance surface — the conformance-only subset — dropping cli.py 247 and mcp_server.py 101, neither of which the spec requires — is **639** LOC). Tests and `bench/` stay outside. Flagged for Sarib as a freeze-blocking judgement call | 🟠 open |
| RM24 | **Stale derived index** — `Doc` caches kids/slugs/edge-adjacency and writers must call `touch()`; there is no single mutation chokepoint (parser, ops, importer all write directly). A stale index corrupts *query results*, and query results are the sole addressing mechanism for edits (D-033), so an agent would edit the wrong node | M | H | E (impl) | canon/walk disagree with a from-scratch rebuild; a node invisible after create | Review cannot be the control. `impl/tests/test_index_fuzz.py` fuzzes random op sequences and asserts cached index == full rebuild after **every** op (**11,831** ops applied by default — the same command CI runs — zero divergence, with a committed negative control that disables `Doc.touch()` and requires the net to report incoherence, so a passing run cannot mean "cannot fail"). It caught exactly this bug during the work | 🟡 mitigated |
| RM11 | Scale — large KBs blow parse/query/memory | M | M | D (Stage 12) | slow load; whole-file-in-memory required | Partial loading, streaming, section anchors (P6/P14); Stage 12 rendering arch. **This risk LANDED and was fixed:** **Measured 2026-07-28** (`bench/scale-report.md`, gate G9) found parse/canon/walk/query/render all ~N^2.0 (the impl never built the derived index D-044 presumes), unusable by 10k nodes. Now all **~N^1.0–1.2 measured** (1,211→124,841 nodes) against a gate threshold of **1.3**, enforced by **G9** (which fits its own exponent over an escalating 1k→50k ladder, plus capacity ≥30k nodes) so it cannot regress unnoticed. Memory half measured: model ≈13× file size, index ≈12% of model — real but modest. Residual: whole-file-in-memory is still required; streaming/partial load (D-044) remains unbuilt | 🟡 mitigated |
...
| RM14 | An operation violates a model invariant (two homes, dangling edge) | L | H | C (Stage 8) | invariants (§11 Stage 4) breakable by an op sequence | **Implemented + tested (Sprint 1-3):** `ops.apply()` validates all 10 invariants after every op and rejects violations; containment only via create/move; corpus-tested. **Measured 2026-07-28** (`bench/scale-report.md`, gate G9): validation is still **full-document per op**, **Resolved 2026-07-28 (D-065):** op-time validation is now scoped to the touched ids, so a 50-token point edit is flat at ~0.001ms at every size (was ~82ms at 125k). Structural ops stay O(N) via index rebuild (follow-up F1). Full-document validation still runs at `sarib validate` and load. Narrowing it to the touched ids was built, measured as no faster, and reverted (follow-up F1). Known consequence: a doc that already carries an unrelated violation rejects *every* op (follow-up F2) | 🟢 mitigated |
...
| RA5 | Derived-relation query cost too high → tempts materialization | M | M | B (Stage 6) | transitive-closure queries slow on big graphs | Mandatory traversal bounds (D-028) cap closure cost; provenance-marked caches (D-022). **Measured 2026-07-28** (`bench/scale-report.md`, gate G9): the bound capped *output* but not *work* — the graph walk re-sorted the entire edge table per visited node (O(V·E log E)). Now adjacency-indexed and linear (~1.2, baseline ~2.0–2.3); ~0.4s at ~125k nodes / 25k edges. Residual: `select:none` is still output-bounded only (follow-up F3) | 🟡 mitigated |
...
| RA10 | Token-economy win shrinks for large-content nodes (`set-content` replaces whole node) | M | L | C (Stage 8) | big prose nodes cost content-sized edits | Keep nodes block-sized; intra-node splicing later (RM4); still ≪ document regeneration (D-002). **Re-rated 2026-07-28** against P6 per WP8: unchanged. This risk is about the *content size of one node*, which the scale work did not touch — node count and content size are independent axes. `bench/scale-report.md` measures the node-count axis only; the content-size axis remains unmeasured and is the residual | 🟡 mitigated |
| RA11 | Surface glyph tokenization unverified — chosen multi-char sigils (`::`,`[[`,`{#`,`^`) may fragment into many tokens | L | M | D (Stage 10) | grammar sigils cost more tokens than assumed | **Measured 2026-07-19** (`bench/tokenizer-report.md`): all load-bearing sigils single-token on o200k/cl100k/r50k; `{.`/`{#` = 2 (acceptable); B saves 26.9% whole-file vs A. **Open-weight residual CLOSED 2026-07-29** (Llama 3.1 · Qwen 2.5 · Mistral v0.3 · Gemma 2 · DeepSeek-V3 (`bench/tokenizer-open-weight.md`)): all 8 load-bearing sigils single-token on every one except `]]` = 2 on Mistral's 32k SentencePiece (inside the pre-registered ≤2 criterion, and a bracket pair rather than a semantic mark); `{.`/`{#` are *cheaper* here than on GPT (1 token on Gemma 2 and DeepSeek-V3). Cost concentrates in content, not syntax — an ISO date costs 9–13 tokens while `::` never fragments. Tokenizer identity is proven by vocab size + a probe fingerprint, not assumed from the repo name. Remaining gap is unrelated to sigils: the whole-file A-vs-B figure is not reproducible because `examples/B-outline-dense.sarib` was never committed | 🟢 resolved |
