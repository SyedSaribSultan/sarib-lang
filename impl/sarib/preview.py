...
Run:  python -m sarib.preview <file.sarib> [-o out.html] [--no-open]
...
It lives inside the package (not tools/) so the editor extension can render a
.sarib file in any folder via `python -m sarib.preview`, with no repo checkout
and no path configuration. `tools/preview.py` is a shim kept for the old path.
"""
